...
// ============================================================
// FIXED VERSION: every CREATE TABLE step now runs in its own
// try/catch (via the safe() helper) so one failing statement
// never blocks the tables that come after it. This is what was
// causing "no such table: payments" / "no such table: coupon_approvals"
// intermittently — a single failure earlier in the old single
// try/catch block silently skipped every table declared after it.
// ============================================================
function initEcosystemDb() {
  ...
  function safe(label, fn) {
    try {
      ...
      console.error('[ecosystem-db] init step failed (' + label + '): ' + (e && e.message ? e.message : e));
    }
  }

  safe('create stores', function () {
  ...
  ['menu_items', 'cart_sessions', 'cart_items', 'supershop_customers', 'security_flags', 'store_orders', 'order_items'].forEach(function (t) {
    safe('create ' + t, function () {
      var ddl = businessTableDdl(t);
      if (ddl) db.exec(ddl);
    });
  });

  safe('menu_items columns', function () {
    ensureColumn(db, 'menu_items', 'barcode', "TEXT DEFAULT ''");
    ensureColumn(db, 'menu_items', 'stock_qty', "INTEGER DEFAULT -1");
    ensureColumn(db, 'menu_items', 'is_visible_to_customers', "INTEGER DEFAULT 1");
    ensureColumn(db, 'menu_items', 'regular_price', "REAL DEFAULT -1");
  ...
  safe('create user_modules', function () {
  ...
  safe('create coupon_approvals', function () {
