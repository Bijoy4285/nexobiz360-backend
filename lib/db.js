...
async function persistWithRetry(next, attempt) {
  try {
    const col = await getCollection();
    await col.replaceOne({ _id: DOC_ID }, next, { upsert: true });
  } catch (err) {
    console.error("MongoDB writeDb error:", err.message);
    const isTopologyError = /topology|closed|not connected/i.test(err.message || "");
