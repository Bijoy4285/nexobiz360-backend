...
const ADMIN_EMAILS = (process.env.ADMIN_EMAILS || "shahrabbibijoy@gmail.com,weavestackit@gmail.com").split(",").map(e => e.trim());
...
function createSession(db, userId) {
  ...
  // Removed: single-session enforcement. Multiple devices/tabs can now stay logged in simultaneously.
  // Optional cleanup: remove only EXPIRED sessions (not all sessions of this user)
  const now = Date.now();
  db.sessions = db.sessions.filter(session => !session.expiresAt || session.expiresAt > now);
