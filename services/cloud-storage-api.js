...
var { getUserDb } = require('../lib/ecosystem-db');
...
// ==========================================================
// মূল হ্যান্ডলার - server.js থেকে কল হবে
// রিটার্ন করে: true (handled) অথবা false (এই route এর জন্য না)
// ==========================================================
function handleRoutes(urlPath, req, res, user) {
  ...
  if (urlPath === '/api/storage/save' && method === 'POST') {
    return parseBody(req).then(function(body) {
      ...
      if (!key) { json(res, 400, { error: 'key is required' }); return true; }
      try {
...
module.exports = { handleRoutes, saveUserStorage, loadUserStorage, loadAllUserStorage };
